...
ZEN_FUNCTION_PIPE_OBJECT((delimit)(const auto& r, const auto& d)
    if (_p<is_range>(r) and _p<is_range>(d))
    (
        r | zen::transform([&d](const auto& x) { return join(x, d); }) | zen::flat | zen::drop(zen::count(d))
    )
)
...
ZEN_TEST_CASE(delimit_test)
{
    ...
    v1.push_back(inner_v);
    v1.push_back(inner_v);
    std::vector<int> v2 = { 1, 2, 0, 0, 1, 2, 0, 0, 1, 2 };
