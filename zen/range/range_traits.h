...
ZEN_TRAITS(is_boost_range)
{
    template<class R>
    auto requires(R&& r) -> ZEN_VALID_EXPR(
        zen::has_type<
            boost::range_iterator<_t<std::remove_reference<R>>>, 
            is_iterator<boost::mpl::_> >()
    );
}
...
ZEN_TRAITS(is_cpp_range)
{
    template<class R>
    auto requires(R&& r) -> ZEN_VALID_EXPR(
        zen::returns< is_iterator<boost::mpl::_> >(begin(r)),
        zen::returns< is_iterator<boost::mpl::_> >(end(r))
    );
}
...
template<class R, class = ZEN_CLASS_REQUIRES(is_range<R>())>
